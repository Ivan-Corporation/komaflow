    ...
    BlacklistableUpgradeable
{
    ...
    error ZeroAddress();
    error ZeroAmount();
    error BlacklistedAccount(address);
    ...
    function _update(address from, address to, uint256 amount) internal override {
        if (from != address(0) && _isBlacklisted(from)) revert BlacklistedAccount(from);
        if (to != address(0) && _isBlacklisted(to)) revert BlacklistedAccount(to);
